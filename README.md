...
DocFusion 支持对已入库文档进行完整的生命周期管理。在"🗄️ 非结构化文档数据库存储"面板的文档列表中，每条记录提供以下操作：

- **详情**：展示文档基础信息、文本块预览（前 5 块）、实体列表（前 20 个）、字段列表（前 20 个）、质量问题列表（前 20 条）
- **导出**：将该文档的全量数据（元信息、文本块、表格、实体、字段、质量问题）打包为 JSON 文件，保存至 `outputs/` 目录并提供下载链接。不删除数据库记录。
- **出库**：先生成 JSON 数据包，再将该文档及其所有关联数据从 SQLite 库中移除。支持下载数据包后确认操作。
- **删除**：只删除数据库记录，不删除原始上传文件。需二次确认。

相关 API：

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/store/documents/{doc_id}` | 文档详情（含预览数据） |
| POST | `/api/store/documents/{doc_id}/export` | 导出数据包（不删除） |
| POST | `/api/store/documents/{doc_id}/checkout` | 出库（导出 + 可选删除） |
| DELETE | `/api/store/documents/{doc_id}` | 仅删除数据库记录 |
...
`POST /api/evaluate/compare` 将填写输出文件与标准结果对比，返回单元格准确率和行准确率（≥80% 为合格）。
